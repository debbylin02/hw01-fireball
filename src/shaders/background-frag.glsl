...
void main()
{
    ...
    float freq = 0.01f;
    float noise = cnoise(vec4(fs_Pos.xyz * freq, sin(time)));

    vec4 backgroundColor = vec4(mix(env_c1, env_c2, noise), 1.0f);

    float scaledClusterSize = (1.0 / clusterSize) / skyRadius;
    float scaledStarSize = (1.0 / starSize) / skyRadius;

    float scaledClusteredNoise = cnoise(scaledClusterSize * fs_Pos + noiseOffset); 
    float scaledStarNoise = + cnoise(scaledStarSize * fs_Pos);

    float cs = pow(abs(scaledClusteredNoise), (1.0f / clusterStrength)) + scaledStarNoise;
    // float c = clamp(pow(abs(cs), 1.0 / starDensity), 0.0, 1.0) * sin(time); 
